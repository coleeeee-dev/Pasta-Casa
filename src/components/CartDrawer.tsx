  ...
  return <div className="overlay" onMouseDown={(e) => e.target === e.currentTarget && onClose()} role="presentation">
    <aside className="drawer" role="dialog" aria-modal="true" aria-labelledby="cart-title">
      ...
      {items.length === 0 ? <div className="empty-state"><div className="empty-icon"><CartIcon /></div><h3>Tu mesa todavía está vacía</h3><p>Elegí tus pastas favoritas y las vamos reuniendo acá.</p><button className="button button-primary" onClick={onExploreCatalog}>Explorar el catálogo</button></div> : <>
        ...
        <div className="drawer-footer"><button className="text-button danger clear-cart" onClick={clearCart}>Vaciar carrito</button><div className="total-row"><span>Total <small>{formatDozens(count)}</small></span><strong>{formatARS(total)}</strong></div><button className="button button-primary button-wide" disabled={!items.length} onClick={onCheckout}>Proceder con la compra <span>→</span></button></div>
      </>}
    </aside>
  </div>
}
