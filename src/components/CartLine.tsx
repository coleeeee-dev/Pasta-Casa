...
export function CartLine({ item, onSet, onRemove }: Props) {
  return <div className="cart-line">
    ...
    <div className="cart-line-main">
      <div className="cart-line-title"><div><strong>{item.product.nombre}</strong><small>{item.product.variante}</small></div><button className="text-button danger" onClick={onRemove}>Eliminar</button></div>
      <div className="cart-line-bottom">
        ...
        <div className="line-price"><small>{formatARS(item.product.precio)} c/u</small><strong>{formatARS(item.product.precio * item.quantity)}</strong></div>
      </div>
    </div>
  </div>
}
