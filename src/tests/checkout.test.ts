...
describe('checkout', () => {
  ...
  it('genera la vista previa sin solicitudes externas', () => { const fetchSpy=vi.spyOn(globalThis,'fetch'); const order:Order={ code:'PED-20260806-A3F7',customer:{nombre:'Ana',apellido:'Díaz',dni:'12345678',email:'ana@mail.com',reviewed:true},items:[{product:products[0],quantity:2}],total:products[0].precio*2,createdAt:new Date().toISOString() }; const email=simulateOrderEmail(order); expect(email.recipient).toBe('ana@mail.com'); expect(email.subject).toContain(order.code); expect(email.body).toContain('SIMULACIÓN'); expect(fetchSpy).not.toHaveBeenCalled(); fetchSpy.mockRestore() })
})
