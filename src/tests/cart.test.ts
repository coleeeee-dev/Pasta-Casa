...
describe('carrito', () => {
  ...
  it('calcula subtotales', () => { expect(getItemSubtotal({ product:products[0],quantity:3 })).toBe(products[0].precio*3) })
  it('calcula el total general', () => { const items=[{ product:products[0],quantity:2 },{ product:products[1],quantity:1 }]; expect(getCartTotal(items)).toBe(products[0].precio*2+products[1].precio) })
})
